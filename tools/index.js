...
client.on('message', function (topic, message) {
    ...
    if (topic.startsWith('stat')) {
        moesDimmers
            ...
            .map((dimmerName) => { topics[dimmerName].queue(message)});
    ...
    if (topic.startsWith('cmnd')) {
        ...
        affectedDimmers.map((dimmerName) => { topics[dimmerName].increaseIgnoreCounter()});
...
const topic = class {
    ...
    sendSequenceOfClicksToServer() { 
        ...
        if(false && this.messageQueue.length === 1) {
            this.sendRawToServer(this.messageQueue[0]);
        }        
        this.messageQueue = [];
    }
